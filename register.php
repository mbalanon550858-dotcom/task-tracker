...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username)||empty($email)||empty($password)||empty($confirm)) $errors[]='All fields are required.';
    if (!preg_match('/^[a-zA-Z0-9_]{3,30}$/', $username)) $errors[]='Username must be 3-30 chars (letters, numbers, underscore).';
...
<div class="auth-wrapper">
  ...
  <div class="auth-form-side">
    <div class="auth-card">
      ...
      <?php if (!empty($errors)): ?>
        <div class="alert alert-error">⚠️ <ul style="margin:0;padding-left:16px;"><?php foreach($errors as $e): ?><li><?= sanitize($e) ?></li><?php endforeach; ?></ul></div>
      <?php endif; ?>
      <form method="POST" action="register.php">
        <div class="form-group"><label>Username</label><input type="text" name="username" placeholder="e.g. john_doe" value="<?= sanitize($_POST['username']??'') ?>" required></div>
        <div class="form-group"><label>Email Address</label><input type="email" name="email" placeholder="you@example.com" value="<?= sanitize($_POST['email']??'') ?>" required></div>
      ...
      <p style="margin-top:24px;text-align:center;color:var(--muted);font-size:.9rem;">Already have an account? <a href="index.php">Sign in</a></p>
    </div>
  </div>
</div>
